...
def esposicionVacia(matriz, fila, col, longitudPalabra, direccion):
    if direccion == 'horizontal':
        return all(matriz[fila, col + i] == ' ' for i in range(longitudPalabra) if 0 <= col + i < matriz.shape[1])
    elif direccion == 'vertical':
        return all(matriz[fila + i, col] == ' ' for i in range(longitudPalabra) if 0 <= fila + i < matriz.shape[0])
    elif direccion == 'diagonal':
        return all(matriz[fila + i, col + i] == ' ' for i in range(longitudPalabra) if 0 <= fila + i < matriz.shape[0] and 0 <= col + i < matriz.shape[1])
    else:
        return False
...
def rellenarLetras(palabras, filas, columnas):
    ...
    for k in range(len(palabras)):
        ...
        for _ in range(num_intentos):
            ...
            fila, col = np.random.randint(0, matriz.shape[0]), np.random.randint(0, matriz.shape[1])
            ...
            if (direccion == 'horizontal' and col + len(palabras[k]) <= matriz.shape[1]) or \
                ...
                (direccion == 'diagonal' and fila + len(palabras[k]) <= matriz.shape[0] and col + len(palabras[k]) <= matriz.shape[1]):
                    if esposicionVacia(matriz, fila, col, len(palabras[k]), direccion):
                        if direccion == 'horizontal':
                            matriz[fila, col:col+len(palabras[k])] = list(palabras[k].upper())  
                        elif direccion == 'vertical':
                            matriz[fila:fila+len(palabras[k]), col] = list(palabras[k].upper())  
                        elif direccion == 'diagonal':
                            for i in range(len(palabras[k])):
                                matriz[fila+i, col+i] = palabras[k][i].upper()  
                        break
        else:
            print(f"No se pudo colocar la palabra {palabras[k]} en la sopa de letras.")
    letrasAzar(matriz)
    return matriz
...
def ingresar_coordenadas(matriz, coordenadaA, coordenadaB):
    ...
    if not (0 <= x1 < len(matriz) and 0 <= y1 < len(matriz[0]) and 0 <= x2 < len(matriz) and 0 <= y2 < len(matriz[0])):
        print("Coordenadas fuera de los límites de la matriz. Por favor, ingrese coordenadas válidas.")
        return None
...
#Dentro del elif
    ...
    while True:
        ...
        if not categorias:
            print("No hay categorías disponibles. Asegúrese de haber ingresado palabras o defina categorías en el diccionario Palabras.")
            break
        categoria_elegida = input(f"Ingrese una categoría (Escríbela tal cual): {categorias}")
    #Creamos un if para asegurarnos de que la categoría ingresada exista en el diccionario
        ...
        if categoria_elegida in Palabras:
            ...
            palabras_a_encontrar = random.sample(valor, cantidad)
            print()
            ...
            print("La categoría ingresada no está en el diccionario. Por favor, inténtelo de nuevo.") 
...
while cantidad_palabras != 0:
    ...
    if intento_palabra.upper() in [palabra.upper() for palabra in Palabras_ingresadas]:  
        palabra_encontrada = True
        Palabras_ingresadas.remove(intento_palabra.upper())  
